...
void fast() {
 ...
 bool f(int x1,multiset<int> &ms1){
    if(ms1.empty()) return false;
    int x2=*ms1.begin();
    if(x2>x1) return false;
    if(ms1.find(x1)==ms1.end()){
        int t1=x1/2;
        int t2=x1/2 + (x1 & 1);
        return f(t1,ms1) && f(t2,ms1);
    }
...
int mx=*max_element(all(v));
    rep(i,0,mx+1){
        if(s.find(i)==s.end()) {cout<<0<<endl; rr;}
    }
    if(!(is_sorted(all(v)))) {no; rr;}
    int ans=1;
    multiset<int> ms;
    for(int i=0;i<4;i++) ms.insert(v[i]);
    for(int i=4;i<n;i++){
        ms.erase(ms.find(v[i-4]));
        ms.insert(v[i]);
        if(ms.size()==4) {cout<<0<<endl; rr;}
    }
