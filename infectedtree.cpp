...
void dfs1(int i, int parent, map<int, int>& cnt, vector<vector<int>>& adj) {
    if (adj[i].size() == 1 && parent != -1) { // Check if it's a leaf
        cnt[i] = 1;
        return;
    }
...
int dfs(int i,int parent,vector<vector<int>> &adj,map<int,int> &cnt,vector<int> &dp){
    if(adj[i].size()==1 && i!=1) {dp[i]=0; return 0;}
  vector<int> child;
    ...
    if(child.size()==1){
        ...
        int c1=child[0]; int c2=child[1];
        dp[i]=max(dp[c1]+cnt[c2]-1,dp[c2]+cnt[c1]-1);
