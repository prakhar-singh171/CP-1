...
int ans;
int dfs(int i,int len,vector<int> &vis,map<int,vector<int>> &adj){
    vis[i]=1;

    int newlen=len+i;
int tt=0;

    for(auto j:adj[newlen]){
        if(!vis[j]){
            tt++;
            dfs(j,newlen,vis,adj);
        }
    }
    if(tt==0) ans=max(ans,newlen);
...
void solve(){
    ...
    rep(i,0,n){
        adj[v[i]+i].push_back(i);
    }
    vector<int> vis(n+2,0);
    rep(i,0,n){
        if(!vis[i] && (n==(v[i]+i))) dfs(i,n,vis,adj);
    }
    cout<<ans<<endl;
